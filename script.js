document.addEventListener('DOMContentLoaded', () => {
    ...
    setupGlobalNav(); // Setup Nav listeners (Gallery Toggle)
    ...
    async function loadLiveStatus() {
        // Fetch About Page to find 'isLive' entry
        // We need to populate CV to check entries
        const data = await fetchAPI('about-page?populate[cv][populate]=*');
    ...
    async function loadProjectList() {
        ...
        const data = await fetchAPI('projects?populate=thumbnail&sort=year:desc');
    ...
    async function loadProjectDetail() {
        ...
        if (!slug) {
            console.warn("No project slug passed.");
            // Optional: Redirect to index or show message
            return;
        }
        ...
        renderProjectDetail(data[0]);
    }

    async function loadAboutPage() {
        const query = `about-page?populate[cv][populate]=*`;
        const data = await fetchAPI(query);
        if (data) renderAboutPage(data);
    }
